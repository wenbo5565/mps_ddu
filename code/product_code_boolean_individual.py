...
# eta =  {(scen, node): scen_eta.loc[scen, node] for scen, node in itertools.product(scen_eta.index, scen_eta.columns)}
eta = scen_eta * -1
...
h_ub = 1
h_lb = 0
c = pd.Series(0.05 * np.ones(num_nodes), index = np.arange(1, num_nodes + 1)) # 1： 0.1; 
...
C_B = return_p_sufficient_ind(zero_ind = Omega_0_ind, up_ind = Omega_up_ind, 
                              down_ind = Omega_down_ind, sample = scen_xi, 
...
C_B_cum_prob = return_cum_p_d(scen_xi = scen_xi, Omega_up_ind = Omega_up_ind, Omega_down_ind = Omega_down_ind, C_B = C_B, tau = tau, epsilon_ind = epsilon_ind)
